...
def sample_throws_by_score_for_sheets(
    *,
    ...
    return (
        ...
        take_sheet_states(sheet_states, np.asarray(state_indices, dtype=int)),
    )


def combine_throw_datasets(
    *datasets: tuple[Throws, SheetStates],
) -> tuple[Throws, SheetStates]:
    throws_list = [throws for throws, _ in datasets]
    states_list = [states for _, states in datasets]
    return concat_throws(throws_list), concat(states_list)
