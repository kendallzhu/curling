...
def _compute_all_pair_times(x, y, v, cos_t, sin_t, ii, jj):
    ...
    for s in prange(num_sims):
        for p in range(num_pairs):
            ...
            dist = (dx * dx + dy * dy) ** 0.5
            max_closing = v1 + v2
            dec = ((1.0 if v1 > 0.0 else 0.0) + (1.0 if v2 > 0.0 else 0.0)) * _MU_G_NB
            c_lb = dist - 2.0 * _R_NB
            if dec == 0.0:
                t_lb = (
                    (c_lb / max_closing if max_closing > 0.0 else np.inf)
                    if c_lb > 0.0
                    else 0.0
                )
            else:
                ...
                if disc_lb < 0.0:
                    ...
                    t_lb = (
                        r1_lb if r1_lb >= 0.0 else (r2_lb if r2_lb >= 0.0 else np.inf)
                    )
            t_lb *= 0.99
